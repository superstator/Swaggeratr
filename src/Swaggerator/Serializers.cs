...
namespace Swaggerator
{
    ...
    {
        internal static string WriteApi(string servicePath, Type serviceType, Stack<Type> typeStack)
        {
            ...
            using (JsonWriter writer = new JsonTextWriter(sw))
            {
                ...
                writer.WriteValue(string.Format("{0}://{1}/{2}", HttpContext.Current.Request.Url.Scheme, HttpContext.Current.Request.Url.Authority, servicePath));
                writer.WritePropertyName("resourcePath");
                writer.WriteValue(string.Format(servicePath));

                writer.WritePropertyName("apis");
                writer.WriteRawValue(Serializers.WriteApis(Mappers.FindMethods(serviceType, typeStack)));

                writer.WritePropertyName("models");
                writer.WriteRawValue(Serializers.WriteModels(typeStack));
            }
            return sb.ToString();
        }
